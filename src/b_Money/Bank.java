...
public class Bank {
	...
	public void openAccount(String accountid) throws AccountExistsException {
		if (accountlist.containsKey(accountid)) {
			...
			//accountlist.get(accountid); //KTR del -> testOpenAccount powinien rzucić wyjątkiem, nie rzucił
			accountlist.put(accountid, new Account(accountid, this.currency)); //KTR add
		}
	}
...
//		if (accountlist.containsKey(accountid)) { //KTR del -> testDeposit() rzucał NullPointerem
		if (!accountlist.containsKey(accountid)) { //KTR add (brakująca negacja)
	...
	public void withdraw(String accountid, Money money) throws AccountDoesNotExistException {
		if (!accountlist.containsKey(accountid)) {
			...
			Account account = accountlist.get(accountid);
			//account.deposit(money); //KTR del -> przy przeglądaniu metody do testu jednostkowego
			account.withdraw(money); //KTR add
		}
	}
...
//		transfer(fromaccount, this, fromaccount, amount); //KTR del -> testTransfer wykrył, że nie został rzucony wyjątek AccountDoesNotExistException w linii 109 dla przeciążonej metody
	 ...
	 * @param tobank Bank where receiving account resides
	 * @param toaccount Id of receiving account
	 * @throws AccountDoesNotExistException   //KTR add
	 */
	public void addTimedPayment(String accountid, String payid, Integer interval, Integer next, Money amount, Bank tobank, String toaccount) throws AccountDoesNotExistException {
		if (!accountlist.containsKey(accountid) || !tobank.accountlist.containsKey(toaccount)) throw new AccountDoesNotExistException(); //KTR add -> testTimedPayment -> powinien rzucać wyjątkiem AccountDoesNotExistException
		Account account = accountlist.get(accountid);
	...
	public void removeTimedPayment(String accountid, String id) throws AccountDoesNotExistException{
		if(!accountlist.containsKey(accountid)) throw new AccountDoesNotExistException(); //KTR add -> testTimedPayment -> powinien rzucać wyjątkiem AccountDoesNotExistException
